		...
		enum power_supply_property prop)
{
	...
	switch (prop) {
		case POWER_SUPPLY_PROP_CAPACITY:
//		case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL: DEBUG
		...
		const union power_supply_propval *val)
{
	...
	switch (prop) {
		case POWER_SUPPLY_PROP_CAPACITY:
			break;
		//case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL: DEGUG
//			break;
		case POWER_SUPPLY_PROP_CHARGE_FULL:	
			break;
		default:
		...
		const struct i2c_device_id *id)
{
	...
	chip->vadc_dev = 0;//qpnp_get_vadc(chip->dev, "lc"); DEBUG
	...
	chip->lc709203_batt_psy.type		= POWER_SUPPLY_TYPE_BATTERY;//POWER_SUPPLY_TYPE_YL_BATTERY; DEGUG
...
MODULE_AUTHOR("ON INC");
MODULE_DESCRIPTION("LC709203 FUELGAUGER IC DRIVER FOR ON Y1");
